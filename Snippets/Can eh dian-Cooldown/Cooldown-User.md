...
{if;{$cooldownArr};includes;{user.id};
    ...
    {=userCdTime;{$user.1}}
    {if;{$userCdTime};>;{time};
        {return;{responder.error;You are on cooldown, you can use this command {time format=relative_time;{$user.1}}}};
        {=cooldownArr;{replace;{$cooldownArr};/{user.id}\-{$userCdTime}\;?/g;}}
    }
}
