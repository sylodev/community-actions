```// replace the {responder.text} at the bottom of the script (line 22) with your code and read the note (line 18+19)
...
{if;{catch;{time;{$cooldownTime}};err};==;err;{return;{responder.error;The creator of the action has input an improper time as a cooldownTime. check out https://docs.atlas.bot/tags/global#time-formattime for valid time inputs}}}
...
// keep the 2 lines of code below together with your output if you want the cooldown to only trigger when a user succesfully uses the command
